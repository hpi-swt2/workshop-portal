class ApplicationController < ActionController::Base
  ...
  def index
    # Application-side filtering isn't all that great, but
  ...
  def add_missing_permission_flashes
    if current_user
      ...
      current_user.events_with_missing_agreement_letters.each do |event|
        ...
        next unless current_user.older_than_required_age_at_start_date_of_event?(event, current_user.profile.age) && application_letter.accepted? && event.acceptances_have_been_sent?
