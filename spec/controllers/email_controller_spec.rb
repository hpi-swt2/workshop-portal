...
RSpec.describe EmailsController, type: :controller do
  ...
  describe "GET #show" do
    ...
    context "with valid accepted applications" do
      before :each do
        @application = FactoryGirl.create(:application_letter, :accepted, event: @event)
        @template = FactoryGirl.create(:email_template, :acceptance)
      end

      it "sets @email with the email of the accepted application" do
        get :show, params: {event_id: @event.id, status: :acceptance}
    ...
    context "with valid rejected applications" do
      before :each do
        @application = FactoryGirl.create(:application_letter, :rejected, event: @event)
        @template = FactoryGirl.create(:email_template, :rejection)
      end

      it "sets @email with the email of the rejected application" do
        get :show, params: {event_id: @event.id, status: :rejection}
        expect(assigns(:email)).to be_a(Email)
        expect(assigns(:email).recipients).to eq(@application.user.email)
      end
  ...
  describe "POST #submit_application_result" do
    context "with valid email pressing the send button" do
      ...
      it "sends an Email" do
        expect{
          post :submit_application_result, params: {send: I18n.t('.emails.email_form.send'), event_id: @event.id, email: @email}
        }.to change{ActionMailer::Base.deliveries.count}.by(1)
      end

      it "sends an Email with ical and agreement pdf attachement for accepted applications" do
        post :submit_application_result, params: {send: I18n.t('.emails.email_form.send'), event_id: @event.id, email: @email, status: :acceptance}
      ...
      it "does not send an Email with ical attachement for rejected applications" do
        post :submit_application_result, params: {send: I18n.t('.emails.email_form.send'), event_id: @event.id, email: @email, status: :rejection}
    ...
    context "with valid email pressing the save template button" do
      ...
      it "saves the current email as template" do
        expect {
          post :submit_application_result, params: {save: I18n.t('.emails.email_form.save_template'), event_id: @event.id, email: @email, status: :acceptance}
