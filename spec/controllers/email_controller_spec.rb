...
RSpec.describe EmailsController, type: :controller do
  ...
  describe "GET #show" do
    ...
    context "with valid accepted applications" do
      before :each do
        @application = FactoryGirl.create(:application_letter, :accepted, event: @event, user: FactoryGirl.build(:user))
    ...
    context "with valid rejected applications" do
      before :each do
        @application = FactoryGirl.create(:application_letter, :rejected, event: @event, user: FactoryGirl.build(:user))
  ...
  describe "POST #submit_application_result" do
    context "with valid email pressing the send button" do
      ...
      it "sends an Email" do
        expect{
          post :submit_application_result, send: I18n.t('.emails.email_form.send'), event_id: @event.id, email: @email
