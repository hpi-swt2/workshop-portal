  ...
  describe "GET #show" do
    ...
    context "with valid accepted applications" do
      before :each do
        ...
        @template = FactoryGirl.create(:email_template_pre_accepted)
      end

      it "sets @email with the email of the accepted application" do
        get :show, event_id: @event.id, status: :pre_accepted
  ...
  describe "POST #submit" do
    ...
    context "with valid email pressing the save template button" do
      ...
      it "saves the current email as template" do
        expect {
          post :submit, save: I18n.t('.emails.email_form.save_template'), event_id: @event.id, email: @email, status: :pre_accepted
